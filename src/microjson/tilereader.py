...
class TileReader(TileHandler):
    ...
    def tiles2microjson(self,
        zlvl: int=0) -> dict[str, Union[str, int]]:
            """
            ...
            Get the bounds from the tilejson and use to generate the MicroJSON data,
            by reading the tiles at the specified zoom level and extracting the
            geometries from the tiles.
            ...
            if (self.tile_json.minzoom is None or zlvl < self.tile_json.minzoom or
                self.tile_json.maxzoom is None or zlvl > self.tile_json.maxzoom):
                return {}
            ...
            def project(coord, xmin, ymin, xmax, ymax, maxy,
                        extent=4096):
                return [
                    (coord[0] / extent * (xmax - xmin) + xmin),
                    (coord[1] / extent * (ymax - ymin) + ymin)
                    #maxy - (coord[1] / extent * (ymax - ymin) + ymin)
                ]
            ...
            microjson_data: dict[str, Union[str, list]] = {
                ...
                for y in range(ntiles):
                    ...
                    if 'features' in tile_data.keys():
                        for feature in tile_data['features']:
                            ...
                            # please note that the coordinates may be in up to 5 nested lists
                            ...
                            if 'geometry' in feature:
                                ...
                                if 'type' in geom:
                                    if geom['type'] == 'Point':
                                        ...
                                        geom['coordinates'] = [project(
                                            coord, xstart, ystart, xstop, ystop) for coord in geom['coordinates']]
                                    elif geom['type'] == 'Polygon':
                                        geom['coordinates'] = [[project(
                                            coord, xstart, ystart, xstop, ystop, maxy
                                            ) for coord in ring] for ring in geom['coordinates']]
                                    elif geom['type'] == 'MultiPolygon':
                                        geom['coordinates'] = [[[project(
                                            coord, xstart, ystart, xstop, ystop) for coord in ring] for ring in poly] for poly in geom['coordinates']]
                                    else:
                                        continue

                                # add the feature to the microjson data
                                microjson_data['features'].append(feature)


                    #except Exception as e:
                    #    logger.error(f"Error processing tile {x},{y},{zlvl}: {str(e)}")
                    #    continue
